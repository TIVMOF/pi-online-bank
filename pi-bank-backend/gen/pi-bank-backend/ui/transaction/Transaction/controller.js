angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'pi-bank-backend-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "transaction" && e.view === "Transaction" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("Transaction", `Unable to count Transaction: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Transaction", `Unable to list/filter Transaction: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function () {
			...
			messageHub.showDialogAsync(
				'Delete Transaction?',
				`Are you sure you want to delete Transaction? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("Transaction", `Unable to delete Transaction: '${response.message}'`);
							return;
						}
						refreshData();
						$scope.loadPage($scope.dataPage, $scope.filter);
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("Transaction-filter", {

...
export class BankAccountTypeRepository {
    ...
    private async triggerEvent(data: BankAccountTypeEntityEvent | BankAccountTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("pi-bank-backend-Settings-BankAccountType", ["trigger"]);
        ...
        producer.topic("pi-bank-backend-Settings-BankAccountType").send(JSON.stringify(data));
    }
}

...
export class CardTypeRepository {
    ...
    private async triggerEvent(data: CardTypeEntityEvent | CardTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("pi-bank-backend-Settings-CardType", ["trigger"]);
        ...
        producer.topic("pi-bank-backend-Settings-CardType").send(JSON.stringify(data));
    }
}

...
export class BankAccountStatusRepository {
    ...
    private async triggerEvent(data: BankAccountStatusEntityEvent | BankAccountStatusUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("pi-bank-backend-Settings-BankAccountStatus", ["trigger"]);
        ...
        producer.topic("pi-bank-backend-Settings-BankAccountStatus").send(JSON.stringify(data));
    }
}

...
export class BankAccountTypeRepository {
    ...
    private async triggerEvent(data: BankAccountTypeEntityEvent | BankAccountTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("bank-backend-Settings-BankAccountType", ["trigger"]);
        ...
        producer.topic("bank-backend-Settings-BankAccountType").send(JSON.stringify(data));
    }
}

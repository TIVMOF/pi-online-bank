...
export class BankAccountStatusRepository {
    ...
    private async triggerEvent(data: BankAccountStatusEntityEvent | BankAccountStatusUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("bank-backend-Settings-BankAccountStatus", ["trigger"]);
        ...
        producer.topic("bank-backend-Settings-BankAccountStatus").send(JSON.stringify(data));
    }
}

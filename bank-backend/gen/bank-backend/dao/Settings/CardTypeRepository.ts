...
export class CardTypeRepository {
    ...
    private async triggerEvent(data: CardTypeEntityEvent | CardTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("bank-backend-Settings-CardType", ["trigger"]);
        ...
        producer.topic("bank-backend-Settings-CardType").send(JSON.stringify(data));
    }
}
